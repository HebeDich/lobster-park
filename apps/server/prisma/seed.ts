...
async function main() {
  ...
  const browserBridgeCliPath = resolve(process.cwd(), 'packages', 'browser-bridge-cli');
  let browserBridgeContentJson: Record<string, unknown>;
  try {
    const skillContent = JSON.parse(readFileSync(resolve(browserBridgeCliPath, 'skill-content.json'), 'utf8'));
    const cliScript = readFileSync(resolve(browserBridgeCliPath, 'browser-bridge.js'), 'utf8');
    browserBridgeContentJson = { ...skillContent, files: { ...skillContent.files, 'browser-bridge.js': cliScript } };
  } catch {
    browserBridgeContentJson = { type: 'prompt_injection', injectionMode: 'append', systemPromptAppend: '浏览器桥接工具（CLI 文件加载失败，请检查 packages/browser-bridge-cli/）' };
  }
  await prisma.skillPackage.upsert({ where: { id: 'skl_browser_bridge' }, update: { contentJson: browserBridgeContentJson }, create: { id: 'skl_browser_bridge', sourceType: 'builtin', sourceUri: 'builtin://browser-bridge', version: '1.0.0', reviewStatus: 'approved', riskLevel: 'medium', tenantPolicyEffect: 'allow', metadataJson: { name: '浏览器桥接', description: '让 Agent 通过浏览器桥接扩展控制用户本地浏览器' }, contentJson: browserBridgeContentJson, createdBy: userId } });
