...
| 迁移 | Prisma Migrate（替代 Flyway） | 与 Prisma ORM 原生集成；SQL-first 原则不变 |
| 进程管理 | PM2 | OpenClaw 实例进程的守护与监控 |
| 日志 | Pino | 结构化 JSON 日志，性能优秀 |
| API 文档 | 基于 OpenAPI V1.2 生成 | NestJS Swagger 插件自动同步 |

### 4.4 模块间调用规范

```ts
// 模块间通过接口调用，不直接引用实现类
// 示例：instance-service 依赖 config-service
...
| `alert_rules` | V1 内置告警规则（详见第 8 节） | 见下文 |
...
1. outbox pending 记录 < 100 条：正常降级，所有操作可继续
2. outbox pending 记录 ≥ 100 条：阻塞高危操作（删除实例、发布配置、强制发布、节点审批、角色变更）
